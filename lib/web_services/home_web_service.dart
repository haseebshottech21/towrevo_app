...
class HomeWebService {
  ...
  Future<List<CompanyModel>> getCompaniesList(Map<String,String> body) async{
   ...
   final response = await http.post(Uri.parse(Utilities.baseUrl+'companies'),
        body: {
          'longitude' : '67.059928',
          'latitude' : '24.871800',
          'time' : '16:08',
  ...
  Future<void> sendRequestToCompany(String serviceId,String companyId,String notificationId)async{
    final response = await http.post(
      Uri.parse(Utilities.baseUrl+'request-service',),
      headers: await Utilities().headerWithAuth(),
      body: {
  ...
  Future<void> sendNotification(String title, String body, String fcmToken) async {
    ...
    const fcmKey = "AAAAaZTXvTc:APA91bHoOPCTghnb4tifuy3ZQBCuEKJvapyQGKk3BFpj_Ec5LNutNv-dH3rYXAHTaTKjuRkxcEIuszj3JonwlYE-LF9aPQK4VOvIwlZHxXiWwvYQxcmIcXjoviJwa9PqqcvkQ9fEMRNs";
    var headers = {'Content-Type': 'application/json', 'Authorization': 'key=$fcmKey'};
    var request = http.Request('POST', Uri.parse(fcmUrl));
    request.body = '''{"to":"$fcmToken","priority":"high","notification":{"title":"$title","body":"$body","sound": "default"},"data":{"screen":"requests"}}''';

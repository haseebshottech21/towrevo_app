...
class _GetLocationScreenState extends State<GetLocationScreen> {
  ...
  Widget build(BuildContext context) {
    final locationViewModelProvider = Provider.of<GetLocationViewModel>(context,listen: false);
    return  Scaffold(
      ...
      body: GoogleMap(
        initialCameraPosition: CameraPosition(target: locationViewModelProvider.latLng!,zoom: 0),
      ...
      floatingActionButton: FloatingActionButton(onPressed:(){
        _googleMapController!.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target:   locationViewModelProvider.latLng!,),),);
        _addMarker( locationViewModelProvider.latLng!);
  ...
  void _addMarker(LatLng pos){
    _origin =  Marker(markerId: const MarkerId('origin'),infoWindow: const InfoWindow(title: 'Origin'),icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueBlue),position: pos);

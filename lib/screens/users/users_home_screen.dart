...
class _UsersHomeScreenState extends State<UsersHomeScreen> {
        ...
        child: Stack(children: [
          ...
          Container(
            ...
            child: Column(children: [
              ...
              Consumer<GetLocationViewModel>(builder: (ctx,getLocation,neverBuildChild){
                return InkWell(
                  onTap: ()async{
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Container(padding: const EdgeInsets.symmetric(vertical: 8),width: MediaQuery.of(context).size.width*0.65,child: Text(getLocation.getAddress.isEmpty?'Get Location' : getLocation.getAddress,style: GoogleFonts.montserrat(color: Colors.black,),maxLines: 3,overflow: TextOverflow.ellipsis,)),
                          ],
                        ),
                        Icon(Icons.my_location,color: primaryColors,),
              ...
              Consumer<ServicesAndDaysViewModel>(builder: (ctx, service, neverBuildChild) {
                return Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      dropdownColor: const Color(0xFF019aff).withOpacity(0.9),
                      items: service.serviceListViewModel.map((ServicesModel value) {
                        return DropdownMenuItem<String>(
                          value: value.name,
                          child: Text(value.name),
                        );
                      }).toList(),
                      onChanged: (value) => service.changeServiceSelectedValue(value!),
                    ),
                  ),
                );
              },
  ...
  void navigateUserHomeScreen() async{
    ...
    final userHomeScreenProvider = Provider.of<UserHomeScreenViewModel>(context,listen: false);
    DateTime now = DateTime.now();
    final lngLatProvider = Provider.of<GetLocationViewModel>(context,listen: false);
    ...
    userHomeScreenProvider.body ={
      ...
      'service' : serviceProvider.serviceListViewModel.firstWhere((element) => element.name == serviceProvider.serviceSelectedValue!).id

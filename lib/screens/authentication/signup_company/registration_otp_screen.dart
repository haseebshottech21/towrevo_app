...
class _RegistrationOTPScreenState extends State<RegistrationOTPScreen> with TickerProviderStateMixin{
  ...
  void sendOTPRequest(bool requestFromCompany) async{
    if(isTimeAvailable){
      if(inputOTP.length == 5){
        ...
        bool response = await provider.sendOTP(provider.resendUniqueId,inputOTP,context);
        ...
        if(!requestFromCompany && response){
          ...
          //company entry
          Navigator.of(context).pushNamedAndRemoveUntil(CompanyHomeScreen.routeName, (route) => false);
        }else if(!response){
  ...
  void resendOTPRequest()async{
    ...
    final provider = Provider.of<OTPViewModel>(context,listen: false);
    await provider.resendOTP(provider.resendUniqueId);;
    // _controller.forward();
  }
